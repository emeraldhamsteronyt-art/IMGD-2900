...
var lonerImage, lonerSprite, spiralImage, spiralSprite, lifeCube, x, y;
/*
...
PS.keyDown = function( key, shift, ctrl, options ) {
	...
	if ((key == PS.KEY_ARROW_RIGHT || key == 100) && x<31) {
		x = x+1;
		PS.spriteMove( lifeCube, x, y );
	}

		if ((key == PS.KEY_ARROW_LEFT || key == 97)&& x>0) {
		x = x-1;
		PS.spriteMove( lifeCube, x, y );
	}

			if ((key == PS.KEY_ARROW_UP || key == 119)&& y>0) {
		y = y-1;
		PS.spriteMove( lifeCube, x, y );
	}

			if ((key == PS.KEY_ARROW_DOWN || key == 115)&& y<31) {
		y = y+1;
		PS.spriteMove( lifeCube, x, y );
	}
