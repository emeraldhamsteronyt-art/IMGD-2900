...
PS.audioLoad("click", {path : "assets/audio/"} );
PS.audioLoad("Hover", {path : "assets/audio/"} );
PS.audioLoad("spiraling", {path : "assets/audio/"} );
...
PS.init = function( system, options ) {
	...
	var didithit = PS.random(4);
		if (didithit > 1){
			...
			if (sanityHP < 1){
			PS.audioPlay("spiraling", {path : "assets/audio/"} );
			gameOver = true;
			PS.gridSize(31, 31);
...
PS.keyDown = function( key, shift, ctrl, options ) {
	...
	if (gameOver == false){
		...
		PS.audioPlay("Hover", {path : "assets/audio/"} );
	...
	if (key == PS.KEY_ENTER  &&  canMove == false) {
		PS.audioPlay("click", {path : "assets/audio/"} );
		switch (menuIndex) {
			case 0:
